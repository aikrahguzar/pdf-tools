...
(defun pdf-view-slice-from-bounding-box (bb)
  ...
  (let* ((margin (/ (max 0 (or pdf-view-bounding-box-margin 0)) 2))
         (bb `(,(max 0 (- (nth 0 bb) margin))
               ,(max 0 (- (nth 1 bb) margin))
               ,(min 1 (+ (nth 2 bb) margin))
               ,(min 1 (+ (nth 3 bb) margin)))))
    `(,(nth 0 bb)
      ,(nth 1 bb)
      ,(- (nth 2 bb) (nth 0 bb))
      ,(- (nth 3 bb) (nth 1 bb)))))
...
This takes `pdf-view-bounding-box-margin' into account."
  (pdf-view-slice-from-bounding-box
   (pdf-cache-boundingbox page)))
...
(defun pdf-view-page-image (&optional page window)
